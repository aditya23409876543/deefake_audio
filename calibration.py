...
class ModelCalibrator:
    ...
    def calibrate_temperature(self, model, val_loader, device='cpu', max_batches=50):
        ...
        self.temperature = torch.nn.Parameter(self.temperature.to(device))  # safe parameter
...
class RobustPredictor:
    ...
    def predict(self, spectral, mfcc=None, phase=None, return_confidence=True):
        """
        Make prediction with calibrated model
        """
        self.model.eval()
        with torch.no_grad():
            # Check if model accepts all 3 features
            is_multi_feature_model = (
                hasattr(self.model, 'spectral_branch') or  # Enhanced model (DeepfakeDetectorCNN)
                hasattr(self.model, 'models') or  # EnsembleDetector, AdaptiveEnsemble
                hasattr(self.model, 'model1') or  # MultiScaleEnsemble
                self.model.__class__.__name__ == 'DeepfakeDetectorCNN' or  # Explicit check
                self.model.__class__.__name__ == 'ResNetGRUModel'  # Open source PyTorch model
            )
            
            if is_multi_feature_model:
                # Enhanced model needs all 3 features
                logits = self.model(spectral, mfcc, phase)
            else:  # Lightweight model
                logits = self.model(spectral)
            
            # Handle single-output models (like PyTorch ResNetGRUModel with num_classes=1)
            is_single_output = logits.shape[1] == 1 if logits.dim() > 1 else logits.dim() == 1
            
            if is_single_output:
                # Binary classification with single logit
                # Use sigmoid to get probability of "fake"
                fake_prob = torch.sigmoid(logits)
                
                # Clip extreme probabilities to prevent false 100% confidence
                epsilon = 1e-6
                fake_prob = torch.clamp(fake_prob, epsilon, 1 - epsilon)
                
                # Create 2-class probabilities: [real_prob, fake_prob]
                probs = torch.cat([1 - fake_prob, fake_prob], dim=1)
                # Prediction: 1 = Fake (>=0.5), 0 = Real (<0.5) - balanced threshold
                prediction = (fake_prob >= 0.5).long()
                # Confidence is probability of predicted class (using clipped values)
                confidence = torch.where(prediction == 1, fake_prob.squeeze(), (1 - fake_prob).squeeze())
                
                # Add warning for extreme confidence on untrained data
                warnings = []
                if torch.max(confidence).item() > 0.99:
                    warnings.append("Very high confidence - verify audio quality")
                
                # Initialize validation variables for single-output
                is_valid = True
            else:
                # Multi-class (original behavior)
                # Apply calibration
                ...
                # Validate prediction
                is_valid, validator_confidence, validator_warnings = self.validator.validate_prediction(probs)
                
                # Use clipped probabilities for confidence, not validator's confidence
                # For multi-class models, use balanced threshold
                fake_probs = probs[:, 1]  # Fake probability
                real_probs = probs[:, 0]  # Real probability
                
                # Use balanced threshold of 0.5 for fairness
                prediction = torch.where(fake_probs >= 0.5, 
                                       torch.tensor(1, device=fake_probs.device), 
                                       torch.tensor(0, device=fake_probs.device))
                
                max_prob = torch.max(probs, dim=1)[0]
                confidence = max_prob
                
                # Merge warnings
                warnings = validator_warnings if 'warnings' in locals() else []
                if max_prob.item() > 0.99:
                    warnings.append("Very high confidence - verify audio quality")
